...
# Introduction
Welcome to the OEX developer documentation. An overview of the merchant docking application development interface.
...
| merchantId | String | Y | Merchant ID. OEX assigned to merchants |
...
After becoming a merchant, the partner needs to log in to the merchant's backend system. Generate RSA public key and private key in the "Merchant Basic Information" menu, and OEX will also provide OEX's public key. Merchants are required to properly keep the public key, private key, and OEX public key. The parameters will be used in subsequent API requests.

### Signature
requestId: request flow id. 20 random characters

merchantId: OEX is assigned to the merchant. It can be viewed in the "Merchant Basic Information" menu of the merchant's backend system.
...
* Remark
    ...
    * The field signature in the response data is generated by the OEX RSA private key signature.
    * For specific implementation, please refer to oex-demo.zip```com.oex.demo.service.GlobalTransferTestService.class#main() method```

### Verify Signature
signature: generated by OEX RSA private key signature
...
* It is composed of all non-empty field contents except the signature field (signature), sorted according to the ASCII code of the message field, and connected with the "&" symbol in the manner of "field name = field value". Signature verification through OEX RSA public key
...
| 500 | Server error. Please contact OEX staff |
...
| Parameter | Type | Required | Meaning |
|----------|--------|----------|:-----|
| orderNo | Long | Y | OEX order number |
...
| orderNo | Long | N | OEX order number |
...
| records[0].orderNo | Long | Y | OEX order number |
